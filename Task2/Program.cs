...
int SumStrings(int[,] massiv, int i)
{
    ...
    Console.WriteLine($"Сумма строки № {NumberRow} = {sumRow}");
...
void MinSumRow(int[,] massiv)
{
    ...
    Console.WriteLine($"Наименьшая сумма ({sumRow}) строки № {NumberRow}");
}

Console.Clear();
Console.Write("Введите размеры двумерного массива n: ");
int n = Convert.ToInt32(Console.ReadLine());
int[,] massiv = CreateArray(n, n);
FillMassiv(massiv);
Console.WriteLine($"Создаем массив размера {n}*{n}", "\n");
Console.WriteLine();
PrintMassiv(massiv);
MinSumRow(massiv);
